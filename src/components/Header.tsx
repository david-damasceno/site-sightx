...
const Header = () => {
  ...
  const menuItems = [
    ...
    { id: 'data-analysis', label: 'Análise' },
  ...
  const NavLink = ({ id, label }: { id: string; label: string }) => (
    <a 
      ...
      className={`font-medium transition-colors relative group ${
        activeSection === id ? 'text-sightx-purple' : 'hover:text-sightx-purple'
      }`}
      onClick={() => setMobileMenuOpen(false)}
    >
      {label}
      <span 
        className={`absolute bottom-0 left-0 h-0.5 bg-sightx-purple transition-all duration-300 ${
        ...
        <div className="flex items-center">
          <a href="#" className="flex items-center">
            ...
            <span className="text-xl font-bold text-black">
              Sight<span className="text-sightx-purple">X</span>
            </span>
          </a>
        </div>
            ...
            {menuItems.map((item) => (
              <a 
                ...
                className={`text-xl font-medium transition-colors ${
                  activeSection === item.id ? 'text-sightx-purple' : 'hover:text-sightx-purple'
                }`} 
                onClick={() => setMobileMenuOpen(false)}
              >
                {item.label}
