...
const Header = () => {
  ...
  return <header className="fixed top-0 left-0 w-full bg-white/90 backdrop-blur-sm z-50 shadow-sm">
      <div className="container-custom flex justify-between items-center h-20">
        ...
        {/* Mobile Menu Button */}
        <button className="md:hidden text-sightx-black" onClick={toggleMobileMenu} aria-label={mobileMenuOpen ? 'Fechar menu' : 'Abrir menu'}>
          {mobileMenuOpen ? <X size={24} /> : <Menu size={24} />}
        </button>
      </div>

      {/* Mobile Navigation */}
      {mobileMenuOpen && <div className="md:hidden fixed inset-0 top-20 bg-white z-40 animate-fade-in">
          <nav className="flex flex-col items-center pt-8 space-y-6">
            ...
            <Button className="btn-primary mt-6" onClick={() => setMobileMenuOpen(false)}>
              Entrar na Lista
            </Button>
          </nav>
        </div>}
